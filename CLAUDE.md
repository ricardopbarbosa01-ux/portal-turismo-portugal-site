...
| beaches/pesca/surf pagination (1E) | 07/05/2026 | All 114+ cards rendered at once on mobile — no pagination. `renderBeaches()`/`renderSpots()` replaces `grid.innerHTML` on every filter change, so a simple init() doesn't survive re-renders. Fixed with `js/beaches-paginate.js` MutationObserver (childList, 60ms debounce) that re-applies PAGE_SIZE=20 after each re-render. | After ANY change to `renderBeaches()` or `renderSpots()` that alters grid innerHTML replacement: verify paginator still fires by checking `data-paginator` button appears. After adding new grid pages: import beaches-paginate.js. |
