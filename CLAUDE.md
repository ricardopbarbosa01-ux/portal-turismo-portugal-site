...
| GYG editorial premium frame redesign | 14/05/2026 | Visual chrome redesign; widget embeds preserved exactly. New classes: .gyg-block-bg, .gyg-block-number, .gyg-block-eyebrow, .gyg-block-frame. Old classes removed: .gyg-eyebrow, .gyg-title, .gyg-trust (without -block- prefix). Commit: ee2d7d7. | After ANY new page with GYG widget: use .gyg-block + .gyg-block-frame template from spec. NEVER touch data-gyg-* attributes. Cache bump gyg-block.css on any CSS edit. pesca pages use boat SVG; all others use waves SVG. Foot CTA only if GYG search URL existed in old block (none did — use text-only foot). |
