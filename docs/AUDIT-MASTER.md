...
| BUG-EN-BEACHES-PAGINATOR | High | RESOLVED | en/beaches.html missing `beaches-paginate.js` script tag — all 100+ cards rendered at once on mobile, no pagination. PT version had script at line 1365; EN was never ported. **Evidence**: script added in commit 6552b2e (11/05/2026); MutationObserver on `#beaches-grid` active; "Show N more" button renders in EN with correct i18n. | session 11/05/2026 |
| BUG-EN-BEACHES-HERO-ZINDEX | High | RESOLVED | en/beaches.html hero text (page-tag, h1, subtitle, CTAs, stats) was NOT wrapped in `.page-header-inner` div — text rendered in normal flow (CSS painting step 3) behind overlay z-index:1 (CSS painting step 7). Text barely visible through 65% dark overlay. Also missing 3 text-shadow rules present in PT. **Evidence**: wrapper div + CSS rule `.page-header-inner{position:relative;z-index:2}` + text-shadows added in commit 6552b2e (11/05/2026). CSS cache v=20260508-en-hero-final. Validated 7/7 markers in production. | session 11/05/2026 |
...
## Fase 6C-D — Entregue 2026-05-06

Fase 6C-D entregue: hero wrapper assimétrico (consistência visual com hero-secondary) + restructure homepage para reduzir scroll. **Hero:** `hero__right` agora com `border-radius: 4px 64px 4px 64px` (desktop) / `4px 40px 4px 40px` (mobile) e `box-shadow: 0 24px 60px -20px rgba(0,0,0,0.4)`. Zero toque no `<video>` element ou URLs CDN. **Homepage restructure:** 10 secções → 7. Nova secção `#what-youll-find` com 3-card grid (Condições do mar · Webcams ao vivo · Surf e Pesca) inserida entre intent e conditions. `planear-cta` agora inclui sub-banda de guias temáticos (5 links `/guias/*`), fundindo a função da `idx-guides-band`. Secções `#conditions`, `#webcams`, `#surf-pesca`, `.idx-guides-band` escondidas via CSS (`display:none`) — HTML 100% preservado para reversibilidade em 1 linha de CSS. CSS cache bumped para `?v=20260506-6cd`. Ficheiros modificados: `css/style.css`, `index.html`, `en/index.html`, `CLAUDE.md`, `docs/AUDIT-MASTER.md`.

## Fase 6C-E — Entregue 2026-05-06

Fase 6C-E entregue: hero principal redesenhado com referência cinematic SVG masking 2026 (Devnahian style). 3 problemas críticos resolvidos: (1) texto "sites de reservas" branco sobre fundo areia = contraste 1:1 → agora navy sólido com underline dourado animado; (2) layout 2018 estático → grid 1fr/1fr cinematic com animação staggered ~2.5s; (3) vídeo confinado a retângulo → SVG clip-path em onda fluida vertical (`clipPathUnits="objectBoundingBox"`). Tipografia outline+solid mix: Fraunces 700 `clamp(36px, 6vw, 84px)`, palavras "O/Portugal/que/nos" em `-webkit-text-stroke 1.5px` vazado, "não/aparece" + "sites de reservas" solid. Animação 9-palavra staggered (delays 0.4s–1.2s) + kicker + sub + CTAs + underline grow. Cursor-reactive parallax via `requestAnimationFrame` (EASE 0.06; `hover:none` skip; `prefers-reduced-motion` skip). iOS Safari fallback: `@media (max-width: 768px)` reset para `border-radius: 4px 60px 4px 60px` + `clip-path: none`. Setinha down (`.hero-scroll`) removida. `prefers-reduced-motion` coberto por regra específica + regra global `animation-duration: 0.01ms !important`. CSS cache bumped para `?v=20260506-6ce`. Ficheiros modificados: `css/style.css`, `index.html`, `en/index.html`, `CLAUDE.md`, `docs/AUDIT-MASTER.md`.

## Fase 6C-F EXPERIMENTAL — branch isolada feature/hero-alt3-experiment

Fase 6C-F EXPERIMENTAL: hero Alt 3 (vídeo intercept + mix-blend-mode: difference) numa branch isolada `feature/hero-alt3-experiment`. Após 6C-E (tipografia outline+solid) ter sido rejeitada como vintage, pesquisa produziu 3 alternativas modernas 2026. Utilizador escolheu Alt 3: headline Fraunces 600 clamp(48px–112px) atravessa o shape do vídeo (clip-path SVG onda), com `mix-blend-mode: difference` + `color: white` — texto aparece quase-preto sobre fundo areia (#f6f4ef ≈ branco) e inverte dinamicamente onde cruza pixels escuros do vídeo. Implementação técnica: hero__media position absolute sem z-index explícito (clip-path cria stacking context); hero__content position relative sem z-index (sem isolamento de stacking context) — headline herda o isolation context de .hero e blends com o vídeo. Fallback @supports: navy sólido + text-shadow. Mobile (<1024px): media passa a relative, border-radius assimétrico, mix-blend-mode desligado. prefers-reduced-motion coberto. Parallax actualizado: apenas hero__media se move (hero__content excluído para não criar stacking context via transform que quebraria o blend). CSS cache bumped para `?v=20260506-6cf-alt3`. Decisão de merge para feature/fetch-and-store-phase-1 depende de validação visual do utilizador. Ficheiros modificados: `css/style.css`, `index.html`, `en/index.html`, `CLAUDE.md`, `docs/AUDIT-MASTER.md`.

## Fase 6C-G — Entregue 2026-05-06

Fase 6C-G entregue: 3 heroes interiores (pesca, surf, webcams) PT+EN com classe `.page-hero` reutilizável aplicando estilo Alt 3. Unsplash hardcoded substituído por fotos Storage próprias. Fotos: PESCA (Pexels, UUID bda7327b), SURF (Wikimedia·Sergey Mysovskiy, UUID 7f6e2b93), WEBCAMS (Pexels, UUID ec40e481). Componente `.page-hero` em css/style.css reusa keyframes heroMediaReveal/heroWordReveal/heroFadeUp + SVG clip-path #hero-wave-clip (objectBoundingBox, onda fluida). mix-blend-mode: difference + Fraunces 600 para headlines. @supports fallback para blend-mode + clip-path. Mobile (<1024px) colapsa para layout standard (clip-path none, border-radius assimétrico). Cursor parallax em todos os 6 ficheiros (EASE 0.06, hover:none + prefers-reduced-motion guards). Fraunces + IBM Plex Mono adicionados via Google Fonts (substituindo DM Sans) em todos os 6 ficheiros. CSS cache `?v=20260506-6cg`. 85→82 ficheiros restantes com Unsplash. Ficheiros modificados: pesca.html, surf.html, webcams.html, en/pesca.html, en/surf.html, en/webcams.html, css/style.css, CLAUDE.md, docs/AUDIT-MASTER.md.
